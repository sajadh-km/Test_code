...
bool quec_usb_serial_create(cops_t *cops, drvUsbWorkMode_t mode)
{
	...
	copsFunc_t *det_funcs[USB_FUNC_ITEM_MAX] = {0};

/*
	用户可以在此函数中自由选择需要使用的端口,不需要的端口直接删除对应行即可;
	usb口很占用ram空间: usb at,usb modem,usb nmea这三个端口每个端口需要128k(tx)+256k(rx)=384k ram
	用户若ram空间紧张,并且要用usb功能,可适量裁减usb口
*/
	...
	{
		det_funcs[0] = createCdcAcmFunc(DRV_NAME_ACM0);
		item_cnt = 1;
	}
	else //USBNET + USBAT + USB_DIAG + USBMOS + USB_CPLOG + USB_APLOG + USB_MODEM + USB_NMEA === 默认usb枚举
	{
		copsFunc_t *funcs[] = {
			(mode == DRV_USB_ECM_AND_SERIALS) ? createEcmFunc() : createRndisFunc(),
			createSerialFunc(DRV_NAME_USRL_COM0),	   //USB AT口,用于AT/数据收发,linux下为ttyUSB0
			createDebugSerialFunc(DRV_NAME_USRL_COM1), //diag口,用于NV校准
			createDebugSerialFunc(DRV_NAME_USRL_COM2), //无实际作用,也不占用
			createDebugSerialFunc(DRV_NAME_USRL_COM3), //usb cp log口
			createDebugSerialFunc(DRV_NAME_USRL_COM4), //usb ap log口
#ifdef CONFIG_QUEC_PROJECT_FEATURE_USB_MODEM_AT
			createSerialFunc(DRV_NAME_USRL_COM5), //USB Modem口,用于AT/数据收发,linux下为ttyUSB5
#endif
#ifdef CONFIG_QUEC_PROJECT_FEATURE_USB_NMEA_AT
			createSerialFunc(DRV_NAME_USRL_COM6), //USB NMEA口,用于AT/数据收发,linux下为ttyUSB6(开启UAC后,此端口将会被枚举为UAC声卡)
#endif
		};
		item_cnt = sizeof(funcs)/sizeof(copsFunc_t *);
		if(item_cnt > USB_FUNC_ITEM_MAX)
		{
			OSI_LOGI(0, "item size error");
			return false;
		}
		memcpy(&det_funcs[0], &funcs[0], sizeof(funcs));
	}
    for (uint8 i=0; i<item_cnt; i++) //检查是否所有usb口数据结构都创建成功
    {
		...
		det_funcs[i]->port_mode = mode;
    }

    if (copsAddFunctions(cops, &det_funcs[0], item_cnt))	//向驱动层注册usb枚举信息
    {
