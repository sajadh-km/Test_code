    ...
    if (res == 0) 
    {
        for (pos = addrInfoList; pos != NULL; pos = pos->ai_next) {
            ...
            } else 
            {
                ...
                timeselect.sec = timeout_ms / 1000;
                timeselect.usec = timeout_ms % 1000 * 1000;
        ...
        uint32_t timeout_ms)
{
    ...
    fd_set recv_sets;
    ql_timeval_t timestart, timenow, timeselect;
    ...
    do {
        ...
        timeselect.sec = timeselect_ms / 1000;
        timeselect.usec = timeselect_ms % 1000 * 1000;
...
static int32_t _core_sysdep_network_udp_recv(core_network_handle_t *network_handle, uint8_t *buffer, uint32_t len, uint32_t timeout_ms, core_sysdep_addr_t *addr)
{
    ...
    fd_set read_fds;
    ql_timeval_t timeout = {timeout_ms / 1000, (timeout_ms % 1000) * 1000};
