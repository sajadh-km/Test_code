...
//mayilian maas
//partition_start_addr标准版本取值固定为0x602F0000。open版本客户需要修改partinfo_8910_8m_opencpu_novolte.json文件对应此值
const quec_bot_hal_logic_partition_t quec_hal_partitions[] =
{
    [QUEC_BOT_PARTITION_ID_KV] =
    {
        .partition_description      = "KV",
#ifndef CONFIG_QL_OPEN_EXPORT_PKG           
        .partition_start_addr       = 0x602F0000,
#else
        .partition_start_addr       = 0x60360000,
#endif
...
quec_bot_hal_logic_partition_t * ql_bot_get_flash_partition(quec_bot_partition_id_e id)
{
    if(id >= QUEC_BOT_PARTITION_ID_MAX)
    {
        return NULL;
    }
    return (quec_bot_hal_logic_partition_t *)&quec_hal_partitions[id];
}
#endif
